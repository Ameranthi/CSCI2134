 ...
 * Part I: IDE detected errors
 * Defect 1: (Bonus mark if they get this)
 *   Line 33-34
 *   Attributes height and width should be private
 *   Any value is fine
 * Defect 2:
 *
 * Defect 3:
 ...
 *
 * getHeight() line 322   assert height == getHeight()
 * getWidth()  line 336     width == getWidth()
 *
 *equals() 118  assert a != null : "Matrix a is null";
        assert (a.getHeight() != height) : "Matrix a is not the same height";
        assert (a.getWidth() != width) : "Matrix a is not the same width";
        *
 * matrix (scanner s) 96 make sure s is int, make sure the elements going into array are doubles
 *
 *
 *
 */
...
public class Matrix {
    ...
    public Matrix(Scanner s) {
        assert s.nextInt() != 0 : "height/width shouldn't be 0";
        assert matrix != null: "matrix shouldn't be null";
        assert s != null : "Shouldn't read in nothings";
        assert s.hasNext() == false: "There needs to be something inputted into scanner to make the matrix";
    ...
    public boolean equals(Matrix a) {
        ...
        assert a != null : "Matrix a shouldn't be null";
        assert (a.getHeight() != height) : "Matrix a is not the same height";
        assert (a.getWidth() != width) : "Matrix a is not the same width";
    ...
    public Matrix add(Matrix b, Matrix res) throws NullPointerException, InputMismatchException {
        ...
        if ((res == null) || (b == null)) {
            throw new NullPointerException("res = " + res + " and b = " + b);
        }

        if( (b.getHeight() != height) || (b.getWidth() != width) ||
                (res.getHeight() != height) || (res.getWidth() != width)) {
            throw new InputMismatchException("height and width for b expected = " + b.getHeight() + "and" + b.getWidth()
                    + " actual width and height "+ height+ " and " + width + " \n height and width for res expected " +
                    res.getHeight() + " and " + res.getWidth() + "actual res height and width " + height + " and " +
                    width);
     ...
     */
    public Matrix multiplyWithScalar(double s, Matrix res) throws NullPointerException, InputMismatchException {
        // if res is not allocated return null

        if ((res == null)) {
            throw new NullPointerException("res = " + res);
        }

        if (res != null) {
            // if matrices are not the same size, return null
            if ((res.getHeight() != height) || (res.getWidth() != width)) {
                throw new InputMismatchException( "height and width for res expected " +
                        res.getHeight() + " and " + res.getWidth() + "actual res height and width " + height + " and " +
                        width);
            }
    ...
    public int getHeight() {
        assert height == getHeight() : "Height is wrong";
     ...
     */
    public int getWidth() {
        assert width == getWidth() : "Width is wrong";
        return width;
    }
}
