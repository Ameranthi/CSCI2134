...
public class District{
  ...
  private int plantCount = 0;                  // current spresso count
  ...
  protected ArrayList<Plant> plants = new ArrayList<>(); // will hold all the plants in an array list
   ...
   */
  public District( DohNat planet, int x, int y, Spresso spresso) {
    myX = x;
    myY = y;
    myPlanet = planet;
    plants.add(spresso);
  ...
  /** 
     ...
     parameter: direction (NORTH, EAST, SOUTH, WEST, or CURRENT 
                ...
                mericano :  array of mericano counts for each adj district
   */
  private void senseDistrict( int direction, int [] spresso, 
    ...
    spresso[direction] = d.earliestHarvest();
  ...
  public void doSensePhase() {
    ...
    if( ( resident != null ) && resident.isFunctional() ) {
      ...
      senseDistrict( CURRENT, plants, bots );
      senseDistrict( NORTH, plants, bots);
      senseDistrict( EAST, plants, bots);
      senseDistrict( SOUTH, plants, bots);
      senseDistrict( WEST, plants, bots);

      // inform timbot of sensor readings.
      resident.senseDistricts( plants, bots );
    }
  }
  ...
  /** 
     ...
     return: number of rounds before plant is ready
      give the minimum amount of rounds a plant will take to fully mature in that district
   */
  public int earliestHarvest() {
  ...
  public String toString() {
    if( ( resident != null ) && resident.isFunctional() ) {
      return String.format( "|%s %2d|", resident.toString(), plantCount);
    } else {
      return String.format( "|          %2d|", plantCount);
    }
  }
}
