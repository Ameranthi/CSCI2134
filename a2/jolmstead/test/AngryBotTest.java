...
class AngryBotTest {
    ...
    AngryBot lilDevil = new AngryBot(20,-1);
    AngryBot sweetandSpicy = new AngryBot(77, 99);
    ...
    void getNextMove() {
        ...
        sweetandSpicy.senseDistricts(spresso, bots);
        ...
        //returns not null
        assertNotNull(sweetandSpicy.getNextMove());

        //make sure ded things don't move when they are not supposed to
        assertEquals(District.CURRENT ,lilDevil.getNextMove(), "The dead bot moved and it was scary");

        //given that we use 1 jolt of the timbot that is dead he should not be functional
        //when
        final boolean result = lilDevil.isFunctional(); //should be false

        //then
        assertFalse(result, "returned that the bot is functional! The bot should be dead!");
        assertTrue(sweetandSpicy.isFunctional(), "This bot should still be functional");
    ...
    void getNextMoveEnergy(){
        ...
        sweetandSpicy.getNextMove();
        int energyAfterMove = sweetandSpicy.energyLevel;
        assertEquals(99, energyAfterMove, "energy was taken despite not moving from the current district");
